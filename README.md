...
| **6주차**<br>2026.04.17 | `Python_Backend_Essentials_for_FastAPI` | - 데코레이터(`@`) 패턴 및 함수 레지스트리 구현<br>- 타입 힌트(Type Hints) — `list[T]`, `Optional`, `Union`<br>- Pydantic `BaseModel`을 활용한 런타임 데이터 유효성 검사<br>- SQLite3 CRUD 실습 및 CSV 데이터 DB 연동<br>- SQLite3 + Pydantic 연동으로 Row 데이터 모델 객체 변환<br>- 동기(Sync) vs 비동기(Async) 파일 다운로드 성능 비교 (`asyncio`, `aiohttp`)<br>- Gradio UI 구현 및 ngrok을 통한 외부 URL 공개 |
